...
HTTP协议是客户机与服务器之间的请求-应答协议，它是建立在TCP/IP协议上的应用层协议
...
web服务器是web资源的宿主。web资源是web内容的源头。最简单的资源是web服务器文件系统中的静态文件如：文本文件，HTML文件，JPEG图片，MP3视频文件等。资源还包含生成动态内容的软件程序如PHP、Pyton等生成的内容。
...
HTTP是一个应用层协议，HTTP不关心网络通信的具体细节，它将联网的细节都交给通用可靠的的TCP/IP协议进行管理。
...
HTTP客户端向服务器发送报文之前，需要用网际协议在客户端与服务端之间通过地址和端口建立一个TCP/IP连接。

这就需要在TCP连接前需要知道服务器的IP地址以及服务器上特定软件相关的端口。
...
第二种URL没有数字形式的IP地址，它使用了一种文本形式的域名或者成为主机名（www.example.com）。
主机名是人性的话的IP地址别称，可以通过一种域名解析服务（DNS）的机制查询出主机名对应的IP地址。
...
- 浏览器将服务器的主机名转换成服务器的 IP 地址; (c) 浏览器将端口号(如果有的话)从 URL 中解析出来; (d) 浏览器建立一条与 Web 服务器的 TCP 连接;
