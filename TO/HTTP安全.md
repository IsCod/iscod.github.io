...
对称加密技术的缺点之一就是发送者和接收者在相互对话之前, 一定要有一个共享的保密密钥. 如果每对通信都需要自己使用私有的密钥, 如果有N个节点, 每个节点都要和其他所有N-1个节点进行安全对话, 总共需要大约N的平方个保密的密钥

### 密钥长度和枚举攻击

保持密钥的挤密状态是很重要的, 在很多情况下, 编/解码算法是众所周知的, 因此密钥就是唯一保密的东西了

好的加密算法会迫使攻击者试遍每一个可能的密钥, 才能破解代码, 用暴力去尝试破解
