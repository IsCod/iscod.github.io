...
HTTP包含了一种内建机制, 可以用WWW-Authorization和Authorization首部向web站点传送用户的相关信息, 一旦登录, 浏览器可以不断地在每条发送这个站点的请求中发送登录信息了, 这样就总是有登录信息可用了

但是登录多个站点是很繁琐的, 小明从一个站点浏览到另一个站点时需要在每一个站点登录, 更糟糕的是, 用户可能要为不同的站点记住不同的用户名和密码. 用户访问其它站点时可能喜欢使用的用户名已经被其他人使用了, 而且不同的站点为用户名和密码设置了不同的规则, 那么用户很可能会很快放弃上网了
