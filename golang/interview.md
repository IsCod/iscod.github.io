...
1. 文件顺序读取（）
1. 写入数据采用0COPY（发送文件描述符）相对于传统的4次拷贝，零拷贝技术减少没必要的拷贝
1. 支持批量处理、压缩
...
事务中的修改，即使没有提交，对其他事务也都是可见的。事务可以读取未提交的的数据，这也被称为脏读

1. 提交读（RC）

一个事务从开始直到提交之前，所做的任何修改对其他事务都是不可见的。这个级别有时候也叫不可重复读

1. 可重复读（RR）

可重复读解决了脏读问题，该级别保证了同一事物中多次读取的同样记录结果是一致的。但是理论上无法解决幻读问题。
幻读是指当某个事物读取某个范围的记录时，另外一个事务有在该范围内插入了新的记录，当之前的事务再次读取该范围的记录时，会产生幻行。
